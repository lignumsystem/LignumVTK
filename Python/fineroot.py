#!python
## \file fineroot.py
#  \namespace Python.fineroot
#  \brief Produce VTK/VTP files for FineRoots project.
#         Read LIGNUM xml file and create VTK/VTP file.
...
class LignumVTKSpline:
    def __init__(self,branch_lss,cylinder:bool,color_name:str,radius_name:str,lgatype_name:str,resolution:int,
        ...
        #List of actors, one for each branch. Will be added to vtkwriter for file output.
        self.renderer = vtk.vtkRenderer()
        #vtkXMLMultiBlockDataWriter writes the output to ParaView compatible '.vtp' file 
        self.vtkwriter = vtk.vtkXMLMultiBlockDataWriter()
    ...
    def write_vtp_file_spline(self,file_name):
        #Reset camera is needed, otherwise the data might be not update for vtkwriter
        self.renderer.ResetCamera()
        mb_set = vtk.vtkMultiBlockDataSet()
        actors = self.renderer.GetActors()
        for (i,actor) in enumerate(actors):
            mb_set.SetBlock(i,actor.GetMapper().GetInput())
        self.vtkwriter.SetFileName(file_name)
        self.vtkwriter.SetInputData(mb_set)
...
class LignumVTKLine:
    ...
    def __init__(self,ts_ls,cylinder:bool,color_name:str,radius_name:str,lgatype_name:str,nsides:int):
        ...
        #vtkwriter writes the output to ParaView compatible '.vtp' file 
        self.vtkwriter = vtk.vtkXMLPolyDataWriter()
