...
the XML formatted tree datasets to produce VTPC[^vtpc] output. 
...
The main VTPC file (.vtpc) is a metadata file containing the names and locations
...
**Example 2**: Create VTPC file from Lignum XML file:
	
	./lignumvtk -input File.xml -output File.vtpc

**Example 3**: Create VTPC file for all trees for the year 20:

	./lignumvtk -input File.h5 -output File.vtpc -year 20
	
**Example 4**: Create VTPC file for the dataset /TreeXML/60/Tree_8:

	./lignumvtk -input File.h5 -output File.vtpc -dataset /TreeXML/60/Tree_8
	
**Example 5**: Create VTPC file for Tree_8 for all growth years in HDF5 file:

	./lignumvtk -input File.h5 -output File.vtpc -dataset Tree_8
	
**Example 6**: Create VTPC file for trees for all years matching the string Tree_11:
...
**Example 2**: Read voxel space dataset from an HDF5 file and create VTS file. 
...
2002, and ISO 15469:2004 standard. For the implementation, see the *CIESGS* class 
in the [CIESky.h](include/CIESky.h) file, and its accompanying software documentation.

### Command line
The `cievtk` command-line is:

	./cievtk [-help|-h] -azimuth|-a <number> -inclination|-i <number> -rad |-r <number> -cie|-c <1-15,16> \
...
the project FineRoots. The `fineroot.py` program processes tree root XML files and creates VTP files
...
The creation of the VTPC files can take several minutes depending on the number of trees
...
These functions were used in simulations and do not prevent the generation of VTPC files.
